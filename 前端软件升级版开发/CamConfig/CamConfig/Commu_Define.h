...
#define GET_CAM_BASICCFG				0x030c01		//获取相机基本配置参数
#define SET_CAM_BASICCFG                0x030c02    //设置相机基本参数
#define GET_CAM_COMPRESSCFG				0x030c03		//获取相机基本配置参数
#define SET_CAM_COMPRESSCFG             0x030c04    //设置相机基本参数
#define GET_CAM_OSDCFG				    0x030c05		//获取相机OSD参数
#define SET_CAM_OSDCFG                  0x030c06   //设置相机OSD参数
#define GET_CAM_NETCFG				    0x030c07	//获取相机网络配置参数
#define SET_CAM_NETCFG                  0x030c08 //设置相机网络参数
#define GET_CAM_PLATECFG				0x030c09	//获取相机网络配置参数
#define SET_CAM_PLATECFG                0x030c0a       //设置相机网络参数
#define GET_CAM_FACECFG			        0x030c0b//获取相机网络配置参数
#define SET_CAM_FACECFG                 0x030c0c       //置相机网络参数
#define GET_CAM_VEDIO_FRONTCFG			0x030c0d//获取相机视频配置参数
#define SET_CAM_VEDIO_FRONTCFG          0x030c0e//设置相机视频配置参数
#define GET_CAM_PLATE_BRTCFG            0x030c0f//获取车牌亮度补偿参数
#define SET_CAM_PLATE_BRTCFG            0x030c10 //设置车牌亮度补偿参数
#define GET_CAM_ABILITYCFG              0x030c11 //获取相机能力集参数
#define SET_CAM_ABILITYCFG              0x030c12 //设置相机能力集参数
#define GET_CAM_IMAGEOVERLAY               0x030c13 //获取相机字符叠加参数
#define SET_CAM_IMAGEOVERLAY           0x030c14 //设置相机字符叠加参数
#define GET_CAM_FLASHCFG                0x030c15 //获取相机闪光灯参数
#define SET_CAM_FLASHCFG                0x030c16 //设置相机闪光灯参数
#define GET_CAM_IO_INCFG                0x030c17 //获取IO输入参数
#define SET_CAM_IO_INCFG                0x030c18 //设置IO输入参数
#define GET_CAM_IO_OUTCFG               0x030c19 //获取IO输出参数
#define SET_CAM_IO_OUTCFG               0x030c1a //设置IO输出参数
#define GET_CAM_IO_SNAPCFG              0x030c1b //获取IO单线圈触发参数
#define SET_CAM_IO_SNAPCFG              0x030c1c //设置IO单线圈触发参数
#define SET_CAM_REBOOT                  0x030c1d //相机远程重启
#define SET_CAM_UPDATE                  0x030c1e //相机远程升级
...
以下是出入口终端预览命令操作的宏定义
********************************************/
#define PREV_CHAN_VALID 0x040001	//询问端口是否有效
#define PREV_CHAN_START 0x040002	//通道开始取流
#define PREV_CHAN_STOP 0x040003	//通道停止取流
#define PREV_CHAN_HEAD 0x040004	//通道取头
...
typedef struct 
{
    ...
    BYTE byProtocolType;    	/*协议类型，值为90*/	
    BYTE byRes1[3];  		/*保留 */
    DWORD dwCheckSum;    	/*检验和*/	
    DWORD dwCommand;		/*客户端请求命令*/
    DWORD dwLocalIP;		/*客户端IP地址*/
    DWORD dwDVRUserID;	/*用户ID */
    BYTE byMACAddr[6];		/*客户端物理地址*/
    BYTE byRes2[2];  		/*保留 */
}INTER_COMM_HEAD, *LPINTER_COMM_HEAD;
...
typedef struct 
{
    DWORD dwLength;  		/*报文总长度*/
    DWORD dwCheckSum;    	/*检验和*/	
    DWORD dwStatic;		/*返回状态值*/
    BYTE byRes[4];  		/*保留 DVR_LOGIN时，用作返回支持的SDK版本号*/
}INTER_RET_HEAD, *LPINTER_RET_HEAD;



/*************************************************
以下是通讯用到的一些相机参数配置的结构体的定义
**************************************************/
...
typedef struct _tagCAM_PLATE_CFG
{
    ...
    NET_VCA_RECT struLaneRect[MAX_LANERECT_NUM]; //识别区域
    BYTE  byRes[32]; //预留
...
typedef struct _tagCAM_VEDIO_CFG
{
    ...
    NET_DVR_WHITEBALANCE struWhiteBalance; //白平衡配置
    BYTE byRes[32];//预留
