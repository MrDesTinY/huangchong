...
BOOL COpMySQL::OpenConn(LPCTSTR szServerName,LPCTSTR szUserName,LPCTSTR szPassword,LPCTSTR szDBName,LPCTSTR szPort)
{
	try
	{
		...
		{
			sprintf_s(szError,"连接MYSQL数据库失败，错误原因:%s",mysql_error(&m_connMysql));
	...
	catch(CException *e)
	{
		...
		sprintf_s(szException,"连接MYSQL数据库时发生异常,错误信息：%s",szError);
...
void COpMySQL::CloseConn()
{
	...
	catch(CException *e)
	{
		...
		char szMsg[512] ={0};
		e->GetErrorMessage(szError,sizeof(szError));
		sprintf_s(szMsg,"关闭MYSQL数据库时发生异常，错误信息:%s",szError);
		OutputDebugString(szError);
	}
...
BOOL COpMySQL::ExecuteSQL(LPCTSTR szSql)
{
	try
	{
		...
		if( iResult!= 0 )
		{
			...
			char szError[512] ={0};
			sprintf_s(szError,"执行SQL语句失败，SQL:%s,错误原因:%s",szSql,mysql_error(&m_connMysql));
			OutputDebugString(szError);
			LeaveCriticalSection((&m_csLock));
			return FALSE;
		}
	...
	catch(CException *e)
	{
		...
		char szMsg[512] ={0};
		e->GetErrorMessage(szError,sizeof(szError));
		sprintf_s(szMsg,"执行SQL语句时发生异常，错误信息:%s",szError);
		OutputDebugString(szError);
		LeaveCriticalSection(&m_csLock);
		return FALSE;
	}
...
MYSQL_RES* COpMySQL::OpenRecordset(LPCTSTR szSql)
{
	try
	{
		...
		{
			sprintf_s(szTmp,"查询记录集失败，SQL:%s,错误原因:%s",szSql,mysql_error(&m_connMysql));
		...
		{
			sprintf_s(szTmp,"读取记录集失败，错误原因:%s",mysql_error(&m_connMysql));
			OutputDebugString(szTmp);
			LeaveCriticalSection(&m_csLock);
			return NULL;
		}
	...
	catch(CException *e)
	{
		...
		e->GetErrorMessage(szError,sizeof(szError));
		char szMsg[512] ={0};
		sprintf_s(szMsg,"读取记录集时发生异常，错误信息:%s",szError);
...
BOOL COpMySQL::ExecuteMulSQL(LPCTSTR szSql[], int iNum)
{
	...
	{
		sprintf_s(szTmp,"SQL[%d]:%s",i,szSql[i]);
		OutputDebugString(szTmp);
	}
#endif
	try
	{
		...
		for(int i = 0;i<iNum;i++)
		{
			...
			{
				sprintf_s(szTmp,"执行SQL语句失败，错误原因:%s",mysql_error(&m_connMysql));
				OutputDebugString(szTmp);
	...
	catch (CException *e)
	{
		...
		char szMsg[512] ={0};
		e->GetErrorMessage(szError,sizeof(szError));
		sprintf_s(szMsg,"执行多条SQL语句时发生异常，错误信息:%s",szError);
		OutputDebugString(szError);
		mysql_rollback(&m_connMysql);
		LeaveCriticalSection(&m_csLock);
		return FALSE;
...
BOOL COpMySQL::ExecuteSQL(CString strSql)
{
	try
	{
		...
		if( iResult!= 0 )
		{
			...
			char szError[512] ={0};
			sprintf_s(szError,"执行SQL语句失败，SQL:%s,错误原因:%s",strSql,mysql_error(&m_connMysql));
			OutputDebugString(szError);
			LeaveCriticalSection((&m_csLock));
			return FALSE;
		}
