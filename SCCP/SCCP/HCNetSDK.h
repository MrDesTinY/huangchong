...
typedef struct 
{
	...
	BYTE	byRecDrawnFrame;               //  录像是否抽帧 0 - 不抽帧 1 - 抽帧
	BYTE	byNetMainDrawnFrame;            //  网传主是否抽帧 0 - 不抽帧 1 - 抽帧
	BYTE	byNetSubDrawnFrame;            //  网传子是否抽帧 0 - 不抽帧 1 - 抽帧
	BYTE	byReserve[7]; 
...
typedef struct 
{
	...
	DWORD dwHardDiskStatic; //硬盘的状态,按位:1-休眠,2-不正常,3-休眠硬盘出错
...
{
	NET_VCA_POLYGON   struRegion; //区域范围
	DWORD dwLeaveDelay;  //无人报警时间，单位：s，取值1-1800
	DWORD dwStaticDelay; //睡觉报警时间，单位：s，取值1-1800
	BYTE  byRes[8]; //保留
...
typedef struct
{
	...
	BYTE byStorageMode;		//0-盘组模式,1-磁盘配额
	BYTE byRes[43];			//保留
...
/*************************************************
函数名:    	NET_DVR_GetLocalIP
函数描述:	获取所有IP， 用于支持多网卡接口
输入参数:   strIP:存放IP的缓冲区
pValidNum:所有有效 IP 的数量
pEnableBind:是否绑定的指针
输出参数:   			
返回值:		HPR_TRUE表示成功,HPR_FALSE表示失败
*************************************************/
...
/*************************************************
函数名:    	NET_DVR_SetValidIP
函数描述:	设定选择使用的IP下标， 下标由NET_DVR_GetLocalIP获取的数组决定
输入参数:   dwIPIndex:所获取的数组的下标
bEnableBind:是否绑定该IP
输出参数:   			
返回值:		HPR_TRUE表示成功,HPR_FALSE表示失败
*************************************************/
...
输入:	lUserID NET_DVR_Login返回值 
        lAlarmOutPort 报警输出口索引 从0开始  0xffff表示全部报警输出口
...
#define NET_DVR_SET_DISK_QUITA_CFG          1279        // 设置磁盘配额信息
