...
#define NET_ITS_REMOTE_DEVICE_CONTROL	5077	//设置同步主机配置
#define NET_ITS_GET_GATEIPC_CHAN_CFG    5078	//获取同步主机配置
#define NET_ITS_SET_GATEIPC_CHAN_CFG	5079	//设置同步主机配置
#define NET_ITS_TRANSCHAN_START         5080    //同步数据服务器
...
typedef struct tagNET_DVR_EVENT_CAPTURE
{
	...
	//	6-10m 7-30m 8-1h 9-12h 10-24h
    NET_DVR_REL_CAPTURE_CHAN struRelCaptureChan[MAX_PIC_EVENT_NUM]; // 数组下标 0 移动侦测触发抓图 1 视频遮挡触发抓图,2 视频丢失触发抓图,数组3表示PIR报警抓图，数组4表示无线报警抓图，数组5表示呼救报警抓图,数组6表示智能抓图
...
typedef struct tagNET_DVR_FIND_PICTURE_PARAM
{
    DWORD  dwSize;         // 结构体大小 
    LONG   lChannel;       // 通道号
    BYTE   byFileType;     // 查找的图片类型:0定时抓图1 移动侦测抓图 2 报警抓图3  报警 | 移动侦测抓图 4 报警 & 移动侦测抓图 6 手动抓图 ,9-智能图片,10- PIR报警，11- 无线报警，12- 呼救报警,，0xa 预览时截图0xff全部类型
...
typedef struct tagNET_ITS_GATE_VEHICLE
{
	...
	BYTE    byRes1;			//保留
	BYTE    byAlarmReason[32]; //自定义报警类型 默认为中文
	...
	BYTE    byCardNo[48]; // 卡号
	DWORD   dwPicNum;		//图片数量（与picGroupNum不同，代表本条信息附带的图片数量，图片信息由struVehicleInfoEx定义	
	NET_ITS_PICTURE_INFO struPicInfo[4]; 		//图片信息,单张回调，最多6张图，由序号区分
	BYTE    byRes3[256];
...
{ 
	BYTE      byGateSiteID[48];    //出入口编号
	BYTE	  byGateInfo[48];  	//出入口信息描述
	...
	BYTE	  byIoMode;           //放行方式 0 内部车放行 1 临时车不放行 2 所有车辆均放行
	BYTE      byRes[129];           //预留
...
typedef	struct  tagNET_ITS_IPC_CHAN_LANE_CFG
{
	...
	BYTE	  byRes[135];
	NET_ITS_GATE_LANE_CFG  stGateLane[4]; //车道配置结构体
...
typedef struct tagNET_DVR_ALARMHOST_ELECTRICITY_STATE
{
	...
	NET_DVR_POWER	struApprentPower[4];	//视在功率 struApprentPower[0]表示第一路视在功率，以此类推
...
{
	DWORD dwSize;
	BYTE	byDevType;	//1-UPS，2-开关电源，3-气体检测系统，4-温湿度传感器，5-空调，6-电量表
	BYTE	byRes1[3];
...
typedef struct tagNET_DVR_ALARMHOST_UPS_LIMIT_VALUE
{
	...
	int iBatterySurply;		//电池后备时间低 限值
	int iBatteryTempratureHigh;//电池温度高限值
	BYTE	byRes[64];			//保留
...
NET_DVR_API LONG __stdcall NET_DVR_TransChanStart(LONG lUserID, DWORD nCommand, void* lpInBuffer, DWORD nInBufferSize, \
			void(CALLBACK *fSerialDataCallBack)(LONG lSerialHandle, char *pRecvDataBuffer, DWORD dwBufSize, DWORD dwUser), void *pUser);

NET_DVR_API BOOL __stdcall NET_DVR_TransSend(LONG lSerialHandle, LONG lDataType, char *pSendBuf, DWORD dwBufSize);
NET_DVR_API BOOL __stdcall NET_DVR_TransStop(LONG lSerialHandle);
...
NET_DVR_API LONG __stdcall NET_DVR_SendRemoteConfig(LONG lHandle, DWORD dwDataType, char *pSendBuf, DWORD dwBufSize);
