...
#define NET_DVR_GET_POE_ADAPTER_TYPE         6124     //获取POE通道网卡类型
#define NET_DVR_SET_POE_ADAPTER_TYPE         6125     //设置POE通道网卡类型
...
typedef struct 
{
	...
	BYTE byRecordType;	//0:定时录像，1:移动侦测，2:报警录像，3:动测|报警，4:动测&报警, 5:命令触发, 6: 智能录像,10-PIR报警，11-无线报警，12-呼救报警，13-PIR|无线报警|呼救报警
	char reservedData[3];
...
typedef struct 
{
	...
	BYTE byRecordType;				/* 录象类型 0:定时录像，1:移动侦测，2:报警录像，3:动测|报警，4:动测&报警 5:命令触发, 6: 智能录像,10-PIR报警，11-无线报警，12-呼救报警，13-PIR|无线报警|呼救报警*/
	char reservedData;
...
typedef struct 
{
	...
	//3-报警|移动侦测 4-报警&移动侦测 5-命令触发 6-手动录像,7－震动报警，8-环境报警，9-智能报警，10-PIR报警，11-无线报警，12-呼救报警
    BYTE byRes[2];
...
typedef struct 
{
    ...
    DWORD dwFileType;//录象文件类型
	   //不带卡号，0xff－全部，0－定时录像,1-移动侦测 ，2－报警触发，3-报警|移动侦测 4-报警&移动侦测 5-命令触发 6-手动录像,7-智能录像，10-PIR报警，11-无线报警，12-呼救报警，13- PIR|无线报警|呼救报警
	 //带卡号，0xff－全部，0－定时录像，1-移动侦测，2－接近报警，3－出钞报警，4－进钞报警，5-命令触发，6－手动录像，7－震动报警，8-环境报警，9-智能报警，10-PIR报警，11-无线报警，12-呼救报警，13- PIR|无线报警|呼救报警
...
typedef struct tagNET_DVR_ALARMIN_PARAM
{
	...
	BYTE    byType;     //防区类型，0:即时防区,1-24小时防区,2-延时防区 ,3-内部防区，4-钥匙防区 5-火警防区 6-周界防区 7-24小时无声防区
	BYTE    byRes1;    
...
typedef struct tagNET_DVR_ALARMHOST_ABILITY
{
    ...
    BYTE    bySupportAlarmInType[16];//支持的防区类型,数组0:即时防区,1-24小时防区,2-延时防区 ,3-内部防区，4-钥匙防区 5-火警防区 6-周界防区 7-24小时无声防区
...
输入:	lSerialHandle NET_DVR_AlarmHostSerialStart返回值
		lPort 从1开始
		lDateType 数据类型 0-表示数据接收正常 1-表示通道不支持透明通道
...
typedef struct tagNET_DVR_SCREENCONTROL_ABILITY
{
	...
	BYTE byInputStreamNum;  //输入源个数 ---设备支持最大输入通道个数
...
typedef struct _tagNET_DVR_OSDCFG
{
    ...
    unsigned char    uByFontColor_y; /*字体颜色Y    0-255*/
    unsigned char    uByFontColor_u; /*字体颜色U    0-255*/
    unsigned char    uByFontColor_v; /*字体颜色V    0-255*/
    unsigned char    uByBackColor_y; /*背景颜色Y    0-255*/
    unsigned char    uByBackColor_u; /*背景颜色U    0-255*/
	unsigned char    uByBackColor_v; /*背景颜色V    0-255*/
...
typedef enum
{
	SWITCH_LAYOUT = 1, 	// 布局切换 默认
		POWER_OFF_DISP,  		// 关闭大屏幕显示
		POWER_ON_DISP,   		// 打开大屏幕显示
}PLAN_OPERATE_TYPE;
...
{
	BYTE  	byValid;      	// 该项是否有效
	BYTE  	byType;       	// 项的具体操作,1布局切换，2-关闭屏幕，3-打开屏幕
	WORD  	wLayoutNo;  	// 布局号
...
{
	DWORD 	dwSize; 
	NET_DVR_PLAN_CFG struPlanCfg[MAX_PLAN_COUNT /*16*/]; 
	BYTE 	byRes[64];
...
NET_DVR_API BOOL __stdcall NET_DVR_SearchFaceDB(LONG lUserID, LPNET_VCA_SEARCH_FACEDB_COND lpInBuf,fSearchDBCallBack cbSearchDBCallBack, void *pUser);
...
{
	DWORD	dwSize;
	BYTE	byValid;	//0表示结构体内的其他参数无效（删除该网络信号源） 1-结构体内的其他参数有效（修改网络信号源参数）
	BYTE	byTransProtocol;			/*传输协议0-TCP 1-UDP*/
	BYTE	byStreamType;			/*码流类型 0-主码流 1-子码流*/
	BYTE	byRes1;
	NET_DVR_IPADDR	struIP;	
	WORD	wPort;	
	BYTE	byChanNum;	//每个通道都当做一个信号源，大屏服务器根据IP端口和通道号来制定设备ID。	
	BYTE	byResolution;	//; 1-CIF 2-4CIF 3-720P 5-1080P
	BYTE	sDeviceName[NAME_LEN];	//设备名称
	BYTE	sUserName[NAME_LEN];	/* 添加的主机登陆帐号 */
    BYTE	sPassword[PASSWD_LEN];	/* 添加的主机密码 */
	BYTE	byRes3[44];	
...
// 76SP设备POE通道连接的网卡类型配置
typedef struct tagNET_DVR_POE_ADAPTER_TYPE
{
	DWORD		dwSize;              	//结构体大小
	BYTE 		byAdapterType;     		//网卡类型0: 内部网卡 1: LAN1	
	BYTE  		byRes1[127];          	//保留
...
{
	DWORD		dwSize;
	BYTE	    byESATAUsage[MAX_ESATA_NUM];		// 数组索引表示第几个eSATA，0：默认录像 1：默认备份	  
	BYTE	    byMiniSASUsage[MAX_MINISAS_NUM];	// 数组索引表示第几个miniSAS，0：默认录像 1：默认备份
	BYTE 		byRes[32];
...
NET_DVR_API BOOL __stdcall NET_DVR_SetGuardMessageCallBack(LONG lAlarmHandle, MSGCallBack  fMessageCallBack, void* pUser);
